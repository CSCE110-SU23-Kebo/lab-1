...
class TestMetaclass(type):
    ...
    def __new__(cls, name, bases, attrs):
        ...
        for case in attrs['cases']:
            fn = cls.generate_test(
                    attrs['basename'], case['input'], case['visibility'], test_weight=case['weight'])
    ...
    @classmethod
    def generate_test(cls, basename, inp, vis, test_weight=0):
        """ Set up test case """
...
def filter_grades(out):
    return out.split('\n', 1)[-1]

def filter_uin(out):
    return out.replace("Enter UIN: ", "")

def filter_numbers(out):
    return out.replace("Enter numbers: ", "")

def filter_counter(out):
    return out.replace("Enter sentence: ", "")
