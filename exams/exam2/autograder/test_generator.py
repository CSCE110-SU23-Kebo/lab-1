...
class TestMetaclass(type):
        ...
        if len(attrs['cases']) == 0:
            ...
            for case in attrs['cases']:
                fn = cls.generate_test_input(
                        attrs['basename'], case['input'], case['visibility'], test_program=case['test_program'],
    ...
    @classmethod
    def generate_test_input(cls, basename, inp, vis, test_program=None, print_input=False, assert_output=True, test_weight=0):
        """ Set up test case """
        ...
        def test_output(self):
            ...
            if assert_output and test_program is None:
                msg = compute_diff(output, output_sample)
                self.assertEqual(output, output_sample, msg=msg)
            else:
                print(output)
...
def gen_test_cases(filename, test_program=None):
    """
    ...
    if test_program:
        for case in cases.values():
            ...
            case['test_program'] = functools.partial(test_program, **kwargs)
...
def build_test_class(filename):
    ...
    cases = gen_test_cases(filename, run_test)
...
# test_{basename}(testCase, **kwargs) - where inputs will be passed into kwargs
...
def filter_palindrome(out):
    anchor_idx = handle_anchor_idx(out, 'Enter words: ')
    return out[anchor_idx:]

def filter_make_team(out):
    startswith_arr = []
    replace_arr = [
    ...
    filtered_out = []
    for line in out_arr:
        skip_line = any([line.startswith(token) for token in startswith_arr])
        if skip_line or (line == '\n' and filtered_out[-1] == '\n'):
            continue

        # replace substrings in line with newlines if previous filtered line
        # is not a newline
        for replace_regex in replace_arr:
            replacement = '\n'
...
def test_make_team(testCase, **kwargs):
    name = 'make_team'
    try:
        from make_team_sample import make_team as mt_sample
    except ImportError:
        print(f"test_{name}_sample: Missing {name}_sample.py (if you see this, it is an error with the autograder.")

    try:
        from make_team import make_team
    except ImportError:
        print(f"test_{name}: Missing {name}.py")

    import random

    names = kwargs.get("names", None)
    team_size = kwargs.get("team_size", None)

    if names is None or team_size is None:
        return
    ...
    random.seed(0)
    result = make_team(names, team_size)
    random.seed(0)
    expected = mt_sample(names, team_size)
...
def filter_accumulate(out):
    anchor_idx = handle_anchor_idx(out, 'Enter numbers: ')
    return out[anchor_idx:]

def test_set_operations(testCase, **kwargs):
    name = "set_operations"
    try:
        from set_operations_sample import set_operation_1 as so1_sample, set_operation_2 as so2_sample, set_operation_3  as so3_sample, set_operation_4 as so4_sample
    except ImportError:
        print(f"test_{name}_sample: Missing {name}_sample.py (if you see this, it is an error with the autograder.")

    try:
        from set_operations import set_operation_1, set_operation_2, set_operation_3, set_operation_4
    except ImportError:
        print(f"test_{name}: Missing {name}.py")

    testCase.assertEqual(set_operation_1, so1_sample)
    testCase.assertEqual(set_operation_2, so2_sample)
    testCase.assertEqual(set_operation_3, so3_sample)
    testCase.assertEqual(set_operation_4, so4_sample)
