...
def gen_test_cases(filename, gen_input=None, run_test=None):
    ...
    if run_test:
        for case in cases.values():
            ...
            case['run_test'] = functools.partial(run_test, **kwargs)
...
def build_test_class(filename):
    ...
    preproc = getattr(sys.modules[__name__], f'preproc_{basename}', None)
    ...
    if preproc:
        def preproc_run(self, *args, **kwargs):
            preproc = getattr(sys.modules[__name__], f'preproc_{basename}', None)
...
def handle_filtering(out, skip, replace):
    ...
    filtered_out = []
    for line in out_arr:
        skip_line = any([line.lstrip().startswith(token) for token in skip])
        if skip_line or (line == '\n' and filtered_out and filtered_out[-1] == '\n'):
            continue
        ...
        for replace_regex in replace:
            replacement = '\n'
            if len(filtered_out) > 0 and filtered_out and filtered_out[-1] == replacement:
                replacement = ''
...
def handle_call(program, sample, *args, seed=0, preprocess=None, postprocess=None, **kwargs):
    random.seed(seed)
...
def filter_students(out):
    skip = ['Choose from the menu',
            '1',
            '2',
            '3',
            '4',
    ...
    return handle_filtering(out, skip, replace)

def preproc_students():
    shutil.copy2('students_default.txt', 'students.txt')

def test_students(testCase, **kwargs):
    name = '_'.join(test_students.__name__.split('_')[1:])
    try:
        from students_sample import add_name as add_n, find_name as find_n, update_name as upd_n, main as ms
    except ImportError as e:
        print(f"test_{name}_sample: Failed to import sample program. Please notify a TA.")
        raise e

    try:
        from students import add_name, find_name, update_name, main
    except ImportError as e:
        print(f"test_{name}: Failed to import your program")
        raise e

    # test add student
    stu_fn = kwargs.get('student')
    stu_nn = kwargs.get('new_name', None)

    def grab_students_txt(*args):
    ...
    if stu_fn:
        with patch("sys.stdout", new=io.StringIO()):
            result, expected = handle_call(add_name, add_n, stu_fn, preprocess=lambda: preproc_students(), postprocess=grab_students_txt)
            header = f"The {add_name.__name__} function wrote an incorrect value to students.txt"
            testCase.assertEqual(result, expected, msg=f"{header}\n\n{compute_diff(result, expected)}")
        preproc_students()

    if stu_fn:
        with patch("sys.stdout", new=io.StringIO()):
            result, expected = handle_call(find_name, find_n, stu_fn)
            header = f"The {find_name.__name__} function wrote an incorrect value to students.txt"
            testCase.assertEqual(result, expected, msg=f"{header}\n\n{compute_diff(result, expected)}")
        preproc_students()

    if stu_fn and stu_nn:
        with patch("sys.stdout", new=io.StringIO()):
            result, expected = handle_call(update_name, upd_n, stu_fn, stu_nn, preprocess=lambda: preproc_students(), postprocess=grab_students_txt)
            header = f"The {update_name.__name__} function wrote an incorrect value to students.txt"
            testCase.assertEqual(result, expected, msg=f"{header}\n\n{compute_diff(result, expected)}")
        preproc_students()
