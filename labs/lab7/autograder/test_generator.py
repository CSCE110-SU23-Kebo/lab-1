...
def simulate_die_game(seed, p1, p2):
    try:
        from die_game_sample import roll_die
    except ImportError as e:
        print(f"gen_die_game: Failed to import sample program. Please notify a TA.")
        raise e

    n_die = 3
    random.seed(seed)
    rg_pr = random.Random(seed)
    ...
    while not meet_req:
        player = turn % 2
        rolls = [roll_die() for _ in range(n_die)]

        if 2 in rolls and not twos:
            twos = True

        # when there is a 2 simulate enter key
        if any([n == 2 for n in rolls]):
            seq.append('')
            if player == 0:
                total0 = 0
            else:
        ...
        # total > 18 and both players have rolled, then end loop
        if (total0 > 18 or total1 > 18) and player == 1:
            meet_req = True
        else:
            inp_pr = 'p' if rg_pr.random() < 0.5 else 'r'
            if inp_pr != 'r':
                turn += 1
            seq.append(inp_pr)
...
def test_die_game(testCase, **kwargs):
    name = 'die_game'
    try:
        from die_game_sample import roll_die as rds, player_turn as pts, main as ms
    except ImportError as e:
        print(f"test_{name}_sample: Failed to import sample program. Please notify a TA.")
        raise e

    try:
        from die_game import roll_die as rd, player_turn as pt, main as m
    except ImportError as e:
    ...
    inp = simulate_die_game(seed, p1, p2)
    for i, char in enumerate(inp):
        if char == '':
            char = "<enter>"
        print(f"Input {i}: {char}")

    result, expected = handle_random_call(rd, rds, seed=seed)
    testCase.assertEqual(result, expected,
            msg=f"The {rds.__name__} function returned an incorrect value ({result} != {expected}).")
    
    with patch("builtins.input", return_value='r'), patch('sys.stdout', new=io.StringIO()):
        result, expected = handle_random_call(pt, pts, "P1", "P2", 0)
        testCase.assertEqual(result, expected,
                msg=f"The {pt.__name__} function returned an incorrect value ({result} != {expected}).")

    with patch("builtins.input", return_value='p'), patch('sys.stdout', new=io.StringIO()):
        result, expected = handle_random_call(pt, pts, "P1", "P2", 0)
        testCase.assertEqual(result, expected,
                msg=f"The {pt.__name__} function returned an incorrect value ({result} != {expected}).")

    def handle_timeout(signum, frame):
        raise Exception("Your function timed out. There may be a infinite loop.")

    it_input = iter(inp)
    next_input = functools.partial(next, it_input)
    with patch("builtins.input", side_effect=next_input), patch("sys.stdout", new=io.StringIO()) as out:
    ...
    testCase.assertEqual(result, expected, msg=compute_diff(result, expected))
