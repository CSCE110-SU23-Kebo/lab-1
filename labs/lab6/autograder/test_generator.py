...
def filter_collection(out):
    startswith_arr = ['***', '1', '2', '3', '4', '5', '6']
    replace_arr = [
            'Choose a menu option: ',
            'How many books would you like to enter: ',
            r'Enter book [0-9]+: ',
            'What is the size of the collection\? ',
            r"Enter the book ISBNs for collection [0-9]+: ",
            "Which collection would you like to delete\? ",
            "Sort books in ascending or descending order of isbn: "]
    out_arr = out.splitlines(True)
    filtered_out = []
    for line in out_arr:
        if any([line.startswith(token) for token in startswith_arr]) or line == '\n':
            continue
        for replace_str in replace_arr:
            replacement = '\n'
            if len(filtered_out) > 0 and filtered_out[-1] == replacement:
                replacement = ''
            line = re.sub(replace_str, replacement, line)
        if len(line) > 0:
            filtered_out.append(line)
    return ''.join(filtered_out)

def filter_textstats(out):
    anchor_idx = handle_anchor_idx(out, 'Enter text: ')
