...
def run_program(filename, inp=None, timeout=1):
    calc = subprocess.Popen(f'python3 -u {filename}'.split(),
                            stdin=subprocess.PIPE,
                            stdout=subprocess.PIPE,
                            stderr=subprocess.PIPE,
                            encoding='utf8')
    return calc.communicate(inp, timeout)
...
def gen_test_cases(filename, gen_input=None, run_test=None):
    ...
    if run_test:
        for case in cases.values():
            ...
            case['run_test'] = functools.partial(run_test, id=case['id'], inp=case['input'], **kwargs)
...
def build_test_class_part2(filename):
    ...
    postproc = getattr(sys.modules[__name__], f'postproc_{basename}', None)
    ...
    if postproc:
        def postproc_run(self, *args, **kwargs):
            postproc = getattr(sys.modules[__name__], f'postproc_{basename}', None)
            return postproc(*args, **kwargs)
...
def test_graph(testCase, **kwargs):
    name = '_'.join(test_graph.__name__.split('_')[1:])
    ...
    output, err = run_program("graph.py", inp=inp, timeout=1)
    testCase.assertTrue(os.path.exists('equations/graph.png'), msg='Missing output plot at equations/graph.png')
    result_image = plt.imread('equations/graph.png')
    if os.path.exists(f'equations-result-{id}'):
        shutil.rmtree(f'equations-result-{id}')
    shutil.move("equations", f"equations-result-{id}")

    output_sample, err_sample = run_program("graph_sample.py", inp=inp, timeout=1)
    expected_image = plt.imread('equations/graph.png')
    if os.path.exists(f'equations-expected-{id}'):
        shutil.rmtree(f'equations-expected-{id}')
    shutil.move("equations", f"equations-expected-{id}")

    testCase.assertTrue(np.array_equal(result_image, expected_image), msg='graph.png does not look as expected.')
